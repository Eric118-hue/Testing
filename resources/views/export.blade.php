             ...
             <div class="p-2 bd-highlight">
                <div class="input-group ">
                    <div class="form-outline" style="width: 100px;">
                        <input type="search" name="" id="form1" placeholder="N° d'ULD" class="form-control" style="height: 30px; font-size: 12px;"/>
                    </div>
            ...
            <thead>
                ...
                @foreach ($ulds as $uld)
                  <tbody>
                    ...
                    <tr>
                        <th scope="row" >{{ $uld->date }}</th>
                        <td style="width: 10px;">{{ $uld->hour }}</td>
                        <td>{{ $uld->name }}  <button onclick="dropdown()" class="dropbtn">drp</button></td>
                        <td>{{ $uld->Origine }}</td>
                        <td>{{ $uld->ActualArrival }}</td>
                        <td>{{ $uld->Registre }}</td>
                        <td>{{ $uld->MRDLocation }}</td>
                        <td>{{ $uld->Handler }}</td>
                        <td>{{ $uld->MRDLabel }}</td>
                        <td>{{ $uld->RegisterCarrier }}</td>
                        <td>{{ $uld->RegisterFlight }}</td>
                        <td>{{ $uld->AttributedCarrier }}</td>
                        <td>{{ $uld->AttributedReceptacles}}</td>
                        <td>{{ $uld->Weight }}</td>
                    </tr>
                  

                {{-- for the colis --}}
                    @foreach ($sacPost as $coli)
                        <tr  class="test" id="colis{{$coli->id_sacPost}}">
                            <th scope="row" ></th>
                            <td style="width: 10px;"></td>
                            <td>
                                @foreach ($sacPost as $item)
                                    <li>{{ $item->id_sacPost}}</li>
                                @endforeach
                            </td>
                            <td></td>
                            <td></td>
...
{{-- AKE3178789 
                        <div class="dropdown">
                            <button onclick="myfunc()" class="dropbtn">drp</button>
                        ...
                        {{-- <ul class="dropdown">
                            <li>AKE3178848 <button onclick="myfunc()" class="dropbtn">drp</button>
